...
export class InternshipTrackerService {
    async getNextTrackerNumber() {
        const pool = getMysqlPool();
        const [rows] = await pool.query(`SELECT COALESCE(MAX(tracker_number), 0) AS max_number FROM internship_tracker`);
    ...
    async createTracker(data) {
        ...
        const [result] = await pool.query(`INSERT INTO internship_tracker (
          tracker_number,
          student_id,
          industry_id,
          start_date,
    ...
    async updateIqacVerification(id, iqac_verification, reject_reason) {
        ...
        if (iqac_verification === 'declined') {
            await pool.query("UPDATE internship_tracker SET iqac_verification = ?, reject_reason = ? WHERE id = ?", [iqac_verification, reject_reason ?? null, id]);
        }
        else {
            await pool.query("UPDATE internship_tracker SET iqac_verification = ?, reject_reason = NULL WHERE id = ?", [iqac_verification, id]);
        }
    ...
    async getTrackerByNumber(trackerNumber) {
      ...
      LEFT JOIN students s ON it.student_id = s.id
      LEFT JOIN internship_industries i ON it.industry_id = i.id
      WHERE it.tracker_number = ?`, [trackerNumber]);
        return rows[0] ?? null;
    }
    ...
    async listAvailableTrackersByStudent(studentId) {
        await ensureTableExists();
        const pool = getMysqlPool();
        const [rows] = await pool.query(`SELECT
        it.id,
      ...
      ORDER BY it.start_date DESC;`, [studentId]);
        return rows;
    }
}
