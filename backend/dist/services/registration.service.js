...
class RegistrationService {
    async ensureRegistrationLoggerTable() {
        const pool = getMysqlPool();
        await pool.query(`CREATE TABLE IF NOT EXISTS Activity_logger (
        ...
        const [columnRows] = await pool.query(`SELECT COLUMN_NAME, IS_NULLABLE
    ...
    async registerForEvent(input) {
        ...
        try {
            ...
            const [eventRows] = await connection.query(`SELECT
          id,
          event_name,
          event_code,
          maximum_count,
        ...
        FOR UPDATE`, [input.eventId]);
            ...
            if (input.studentId) {
                const [duplicateRows] = await connection.query(`SELECT id
          FROM Activity_logger
          WHERE event_id = ?
            AND student_id = ?
            AND status IN ('pending', 'approved')
          LIMIT 1`, [input.eventId, input.studentId]);
                if (duplicateRows.length > 0) {
                    throw new DuplicateRegistrationError();
                }
            }
            const [insertResult] = await connection.execute(`INSERT INTO Activity_logger (
          event_id,
          student_id,
          student_name,
          student_email,
          ...
          status
            ...
            await connection.execute(`UPDATE Activity_Master
        SET
    ...
    async listRegistrations(status) {
      ...
      ORDER BY
        CASE er.status
          ...
          ELSE 2
        END ASC,
        er.created_date DESC`, params);
        return rows.map(mapRow);
    }
    async approveRegistration(registrationId, verifiedBy) {
        await this.ensureRegistrationLoggerTable();
        const connection = await getMysqlPool().getConnection();
        try {
            await connection.beginTransaction();
            const [updateResult] = await connection.execute(`UPDATE Activity_logger
        SET
          status = 'approved',
          ...
          verified_at = NOW()
        WHERE id = ?
          AND status = 'pending'`, [verifiedBy, registrationId]);
    ...
    async rejectRegistration(registrationId, verifiedBy, reason) {
        await this.ensureRegistrationLoggerTable();
        const connection = await getMysqlPool().getConnection();
        try {
            await connection.beginTransaction();
            const [rows] = await connection.query(`SELECT event_id, status FROM Activity_logger WHERE id = ? LIMIT 1 FOR UPDATE`, [registrationId]);
            const existing = rows[0];
            if (!existing) {
                throw new RegistrationNotFoundError(registrationId);
            }
            const wasPending = existing.status === 'pending';
            await connection.execute(`UPDATE Activity_logger
        SET
          status = 'rejected',
          ...
          verified_by = ?,
          verified_at = NOW()
        WHERE id = ?`, [reason, verifiedBy, registrationId]);
            if (wasPending) {
                await connection.execute(`UPDATE Activity_Master
          SET
    ...
    async getRegistrationsByEventId(eventId, status) {
        await this.ensureRegistrationLoggerTable();
    ...
    async getRegistrationsByStudentId(studentId, status) {
        await this.ensureRegistrationLoggerTable();
      ...
      INNER JOIN Activity_Master em ON em.id = er.event_id
      ${where}
      ORDER BY er.created_date DESC`, params);
        return rows.map(mapRow);
    }
}
