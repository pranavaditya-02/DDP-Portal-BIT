...
const createSubmission = async (connection, taskId, activityType, facultyId, remarks) => {
    try {
        const [result] = await connection.execute(`INSERT INTO submissions (task_id, remarks, activity_type, faculty_id, created_at, updated_at)
       VALUES (?, ?, ?, ?, NOW(), NOW())`, [taskId, remarks, activityType, facultyId]);
...
class FacultyActivitiesService {
    ...
    async getEntries(category) {
        ...
        const [rows] = await pool.query(`SELECT t.*, s.task_id AS taskId
       FROM ${definition.tableName} t
       JOIN submissions s ON s.id = t.submission_id
       ORDER BY t.id DESC`);
        return rows;
    }
    async createEntry(category, data) {
        const definition = getDefinition(category);
        ...
        try {
            ...
            const values = [submissionId, ...payload.map((item) => item.value)];
            await connection.execute(`INSERT INTO ${definition.tableName} (${columns.join(', ')}) VALUES (${placeholders})`, values);
