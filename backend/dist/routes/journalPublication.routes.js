...
router.post('/', upload.fields([
    ...
    try {
        ...
        const { student, yearOfStudy, specialLab, paperTitle, authorsNames, totalAuthors, studentAuthorCount, facultyAuthorCount, studentAuthorNames, facultyAuthorNames, dateOfPublication, volumeNumber, issueNumber, issnNumber, doiNumber, pageFrom, pageTo, journalName, publisherName, webUrl, studentAuthorPosition, labsInvolved, paperIndexed, indexedDetails, indexedOtherDetails, impactFactor, impactFactorValue, projectOutcome, sdgGoals, sponsorshipType, sponsorshipAmount, sponsorshipOtherSpecify, interdisciplinary, interdisciplinaryDepartment, otherDeptStudentCount, iqacVerification, } = req.body;
        const required = [
            'student',
            'yearOfStudy',
            'specialLab',
        ...
        const [result] = await connection.query(`INSERT INTO journal_publications (
...
router.get('/', async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`SELECT jp.*, s.student_name, sl.name AS lab_name,
