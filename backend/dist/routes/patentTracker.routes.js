...
router.get('/:id(\\d+)', async (req, res) => {
    try {
        ...
        if (!t)
            return res.status(404).json({ error: 'Patent tracker not found' });
...
router.patch('/:id(\\d+)/iqac', async (req, res) => {
    try {
        ...
        const updated = await patentTrackerService.updateIqacVerification(existing.id, iqacValue, body.reject_reason ?? null);
        if (!updated)
            return res.status(404).json({ error: 'Patent tracker not found' });
        // send email to student if email available and not initiated
        const studentEmail = updated.student_email;
        if (studentEmail && body.iqac_verification !== 'initiated') {
            ...
            const bodyText = `Hello ${updated.student_name ?? 'Student'},\n\nYour patent tracker submission (ID: ${updated.id}) has been ${statusText} by the IQAC team at BannariAmman College.\n\n${body.reject_reason ? `Reason: ${body.reject_reason}\n\n` : ''}If you have any questions, please reply to this email.\n\nIQAC Team\nBannariAmman College`;
            const bodyHtml = `<p>Hello ${updated.student_name ?? 'Student'},</p><p>Your patent tracker submission <strong>(ID: ${updated.id})</strong> has been <strong>${statusText}</strong> by the IQAC team at <strong>BannariAmman College</strong>.</p>${body.reject_reason ? `<p><strong>Reason:</strong> ${body.reject_reason}</p>` : ''}<p>If you have any questions, please reply to this email.</p><p>IQAC Team<br/>BannariAmman College</p>`;
            try {
