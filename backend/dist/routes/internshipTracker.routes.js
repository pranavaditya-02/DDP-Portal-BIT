...
const transformTrackerForResponse = (req, tracker) => ({
    ...tracker,
...
const allowedFileTypes = new Set((process.env.ALLOWED_FILE_TYPES || 'pdf,jpg,jpeg,png,docx,xlsx')
    .split(',')
    .map((type) => type.trim().toLowerCase())
    .filter(Boolean));
const INTERNSHIP_FILENAME_REGEX = /^[0-9]{7}[A-Z]{2}[0-9]{3}-(?:internship-[0-9]{8}|ITI-[0-9]{2}\.[0-9]{2}\.[0-9]{4})\.[a-zA-Z0-9]+$/i;
const INTERNSHIP_FILENAME_ERROR = 'File name must be in the expected format, for example 7376251CS492-internship-04072026.pdf or 201CS111-ITI-08.06.2025.pdf.';
...
const storage = multer.diskStorage({
    ...
    filename: (_req, file, cb) => {
        ...
        const reqAny = _req;
        if (!reqAny._generatedFilenames)
            reqAny._generatedFilenames = new Set();
...
router.patch('/:trackerId/iqac', async (req, res) => {
    try {
        ...
        const tracker = await internshipTrackerService.updateIqacVerification(existingTracker.id, parsed.iqac_verification, parsed.reject_reason);
        if (!tracker) {
            return res.status(404).json({ error: 'Internship tracker not found' });
        }
