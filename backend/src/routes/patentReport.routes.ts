...
const schema = z.object({
  ...
  prior_art: z.string().optional().nullable(),
...
router.post('/', upload.fields([
  ...
  try {
    ...
    if (!yukti) return res.status(400).json({ error: 'Yukti proof is required.' });
    if (parsed.patent_status === 'filed' && !fullDoc) return res.status(400).json({ error: 'Full document proof required for filed status.' });
    if (parsed.patent_status === 'filed' && !cbr) return res.status(400).json({ error: 'CBR receipt required for filed status.' });
    if (parsed.patent_status === 'published' && !publication) return res.status(400).json({ error: 'Publication proof required for published status.' });
    if (parsed.patent_status === 'granted' && !granted) return res.status(400).json({ error: 'Granted proof required for granted status.' });
    ...
    const report = await patentReportService.createReport({
      ...
      prior_art: parsed.prior_art ?? null,
      novelty: parsed.novelty ?? null,
...
router.get('/:id(\\d+)', async (req, res) => {
  try {
    ...
    if (!rec) return res.status(404).json({ error: 'Patent report not found' });
    const transformed = {
      ...rec,
